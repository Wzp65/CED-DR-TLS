...
def same_events_finding(des_dir, k_filter_path, gte_model, db_gte_sent, dataset, keyword, prompt):
    same_event_list = []
    retro_event = dict()
    with open(k_filter_path, "r", encoding="utf-8") as f:
        data_list = json.load(f)
    ...
    for d in tqdm(data_list, desc=f"same events finding:{dataset}-{keyword}"):
        ...
        if len(d["coarse_candidates"]) == 0 and event_time == event_pubtime:
            continue
        ...
        if event_time != event_pubtime:
            if event_id not in retro_event:
                retro_event[event_id] = content
            continue
        ...
        re_data = [re_c for re_c in re_data if re_c["article_id"] != article_id]
    ...
    retro_event_file_path = os.path.join(des_dir, "retrospect_events.json")
    with open(retro_event_file_path, "w", encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    Qwen_model = AutoModelForCausalLM.from_pretrained(args.Qwen_model, device_map="cuda:4")
    '''
    trigger_syntax_list = []
    ...
    for k in tqdm(keyword, desc="event detection"):
        ...
        same_events_finding(des_dir, k_filter_path, gte_model, db_gte_sent, args.dataset, k, prompt)
